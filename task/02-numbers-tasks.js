...
function getAngleBetweenVectors(x1, y1, x2, y2) {
    let composition = x1 * x2 + y1 * y2;
    function module (par1, par2) {
        return Math.sqrt(Math.pow(par1, 2) + Math.pow(par2, 2));
    }
    let module1 = module(x1, y1);
    let module2 = module(x2, y2);
    let cos = composition / (module1 * module2);
...
function getParallelipidedDiagonal(a,b,c) {
    let sqrDiag = Math.pow(b, 2) + Math.pow(c, 2);
    return Math.sqrt(sqrDiag + Math.pow(a, 2));
}
